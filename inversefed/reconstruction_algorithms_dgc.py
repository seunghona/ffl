...
class GradientReconstructor_dgc():
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True):
        ...
        try:
            for trial in range(self.config['restarts']):
                x_trial, labels, layers_selected = self._run_trial(x[trial], input_data, labels, dryrun=dryrun)
                # Finalize
                scores[trial] = self._score_trial(x_trial, input_data, labels, layers_selected)
                x[trial] = x_trial
                if dryrun:
                    break
    ...
    def _run_trial(self, x_trial, input_data, labels, dryrun=False):
        ...
        try:
            ############################### over here ###############################
            ### my add-ons
            dropratio_str = arrange.ratio
            dropratio = float(arrange.ratio)
            ...
            for i in range(len(input_data)):
                grad_ = input_data[i].flatten().tolist()
                varnum = len(grad_)
                grad_ = torch.FloatTensor(grad_)
                topk = torch.topk(torch.abs(grad_), math.ceil(varnum*dropratio))
                threshold.append(topk[0][-1])
            # print("total variable # is {}, top {}, threshold calc done".format(varnum, int(varnum*dropratio)))
            #print(topk)
            #print(len(topk[0]))
            
            for iteration in range(max_iterations):
                closure = self._gradient_closure(optimizer, x_trial, input_data, labels, threshold)
            ...
            pass
        return x_trial.detach(), labels, threshold

    def _gradient_closure(self, optimizer, x_trial, input_gradient, label, threshold):
    ########################################### here as well #####################################
        def closure():
            optimizer.zero_grad()
            ...
            rec_loss = reconstruction_costs([gradient], input_gradient, threshold,
                                            cost_fn=self.config['cost_fn'], indices=self.config['indices'],
                                            weights=self.config['weights'])
            #print("this is {}".format(rec_loss))
...
class FedAvgReconstructor(GradientReconstructor_dgc):
...
def reconstruction_costs(gradients, input_gradient, threshold, cost_fn='l2', indices='def', weights='equal'):
    ...
    for trial_gradient in gradients:
        ...
        count = 0
        for i in indices:
            mask = torch.abs(input_gradient[i]) >= threshold[i]
            #print(input_gradient[i], threshold)
            #print(torch.sum(mask))
            input_gradient_ = input_gradient[i] * mask
            #print(trial_gradient_)
            if cost_fn == 'l2':
                ...
                costs -= (trial_gradient[i] * input_gradient_).sum() * weights[i]
                if input_gradient_.pow(2).sum() * weights[i] == 0:
                    #print("here")
                    count+=1
                    #print(count)
                    continue
                pnorm[0] += trial_gradient[i].pow(2).sum() * weights[i]
                pnorm[1] += input_gradient_.pow(2).sum() * weights[i]
                #print("culprits are {} {}".format(pnorm[0],pnorm[1]))
