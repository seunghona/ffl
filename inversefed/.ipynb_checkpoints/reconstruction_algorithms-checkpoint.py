...
class GradientReconstructor():
    ...
    def _run_trial(self, x_trial, input_data, labels, dryrun=False):
        ...
        try:
            ...
            if method == "dp": 
                dropratio = 1.0
                ### now for the actual dropratio value, which is 1.0 for dp
            ...
            #print(globgrad.keys())
            counter = 0
            for i in globgrad.keys():
                if "running_mean" in i or "running_var" in i or "num_batches_tracked" in i:
                    #print('skipped in {}'.format(i[10:]))
                    continue

                costs = (globgrad[i] * input_data[counter]).sum() * weights[counter]
                #print('costshape = {} {} {}'.format(((globgrad[i] * input_data[counter]).sum()).shape, costs.shape, weights[counter]))
                pnorm[0] = globgrad[i].pow(2).sum() * weights[counter]
            ...
            if method == "random":
                ...
                for i in range(len(scorelist)):
                    layers_selected.append(i)
                layers_selected = random.sample(list(layers_selected), int(len(layers_selected)*dropratio))
           
            
            for iteration in range(max_iterations):
                closure = self._gradient_closure(optimizer, x_trial, input_data, labels, layers_selected)
            ...
            pass
        return x_trial.detach(), labels, layers_selected
